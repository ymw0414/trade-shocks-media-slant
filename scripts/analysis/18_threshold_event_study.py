"""
18_threshold_event_study.py

Probability-threshold robustness check for the extensive margin.
Instead of classifying articles as R/D by the sign of net_slant (P>0.5),
apply stricter thresholds (P>0.6, 0.7, 0.8) using the logistic model's
predicted probability. This filters out "centrist" articles near the
decision boundary.

Outcomes:
  - share_R_{t}: fraction of articles with P(R) > t
  - share_D_{t}: fraction of articles with P(R) < 1-t
  - r_ratio_{t}: R/(R+D) among confident articles (no zero-inflation)
"""
...
def aggregate_by_threshold():
    """Aggregate article-level prob_R to newspaper-year panel at each threshold."""
...
def build_panel(agg):
    """Merge threshold aggregates with regression panel."""
    panel = pd.read_parquet(
        RUN / "panel" / "14_regression_panel.parquet")
...
def run_event_studies(merged, years):
    """Run event studies for each threshold and outcome."""
...
def plot_share_R_by_threshold(all_results, years):
    """Overlay Share R event studies for different thresholds."""
    fig, ax = plt.subplots(figsize=(12, 6))
    colors = {0.5: "#cccccc", 0.6: "#999999", 0.7: "#666666", 0.8: "#333333", 0.9: "#111111"}
    offsets = {0.5: -0.4, 0.6: -0.2, 0.7: 0.0, 0.8: 0.2, 0.9: 0.4}
    for t in THRESHOLDS:
        c = all_results[(t, f"share_R_{t}")]
        yrs = c["year"].values
        ax.errorbar(yrs + offsets[t], c["coef"],
                    yerr=[c["coef"] - c["ci_lo"], c["ci_hi"] - c["coef"]],
                    fmt="o", color=colors[t], markersize=3.5, capsize=2, linewidth=0.8,
                    label=f"P(R)>{t}")
    ...
    fig.tight_layout()
    out = FIG_DIR / "threshold_share_R.png"
    fig.savefig(out, dpi=200, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    print(f"Saved: {out}")


def plot_r_ratio_by_threshold(all_results, years):
    """Overlay R Ratio event studies for different thresholds."""
    fig, ax = plt.subplots(figsize=(12, 6))
    colors = {0.5: "#cccccc", 0.6: "#999999", 0.7: "#666666", 0.8: "#333333", 0.9: "#111111"}
    offsets = {0.5: -0.4, 0.6: -0.2, 0.7: 0.0, 0.8: 0.2, 0.9: 0.4}
    for t in THRESHOLDS:
    ...
    ax.spines["right"].set_visible(False)
    fig.tight_layout()
    out = FIG_DIR / "threshold_r_ratio.png"
    fig.savefig(out, dpi=200, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    print(f"Saved: {out}")


def plot_07_R_vs_D(all_results, years):
    ...
    for idx, (depvar, title, color) in enumerate([
        ...
        ax.spines["right"].set_visible(False)
    fig.tight_layout()
    out = FIG_DIR / "threshold_07_R_vs_D.png"
    fig.savefig(out, dpi=200, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    print(f"Saved: {out}")


def plot_grid(all_results, years):
    """5x3 grid: Share R, Share D, R Ratio at each threshold."""
    ...
    out = FIG_DIR / "threshold_grid_5x3.png"
    fig.savefig(out, dpi=200, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    print(f"Saved: {out}")
...
def main():
    agg = aggregate_by_threshold()
    merged, years = build_panel(agg)

    print(f"\nRegression panel: {len(merged):,} obs")
    print(f"Running event studies...")
    all_results = run_event_studies(merged, years)

    plot_share_R_by_threshold(all_results, years)
    plot_r_ratio_by_threshold(all_results, years)
    plot_07_R_vs_D(all_results, years)
    plot_grid(all_results, years)
