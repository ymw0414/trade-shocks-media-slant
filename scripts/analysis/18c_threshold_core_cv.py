"""
18c_threshold_core_cv.py

Probability-threshold robustness for exp_uni_gst_core_cv.
"""
...
RUN = BASE / "data" / "processed" / "runs" / "exp_uni_gst_core_cv"
...
def main():
    ...
    ax.set_title("Share R-Leaning by Probability Threshold (Core 20%)", fontsize=13, fontweight="bold")
    ...
    ax.set_xlabel("Year", fontsize=12)
    ax.set_ylabel("Coefficient on Vulnerability \u00d7 Year", fontsize=12)
    ax.set_title("R Ratio [R/(R+D)] by Probability Threshold (Core 20%)", fontsize=13, fontweight="bold")
    ax.legend(fontsize=10, framealpha=0.9)
    ax.set_xticks([yr for yr in yrs if yr % 2 == 1 or yr == yrs[0]])
    ax.set_xlim(yrs[0] - 0.7, yrs[-1] + 0.7)
    ax.grid(axis="y", alpha=0.3)
    ...
    for row_idx, t in enumerate(THRESHOLDS):
        for col_idx, (stem, title_stem, color) in enumerate([
            ...
            ax.spines["top"].set_visible(False)
            ax.spines["right"].set_visible(False)
    fig.suptitle("Event Study by Probability Threshold (Core 20%)", fontsize=14, fontweight="bold", y=0.98)
