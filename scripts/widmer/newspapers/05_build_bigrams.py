...
- Input:
    - clean_text from 04a_clean_text.py
    - clean_title from 04b_clean_title.py
- Output:
    - X_body  : bigram counts from body
    - X_title : bigram counts from title (same vocabulary)
    - vocabulary
    - metadata
"""
...
BODY_FILE  = CLEAN_DIR / f"newspapers_congress_{CONGRESS}_clean.parquet"
...
def main():
    ...
    print("Task: Build BODY and TITLE bigram matrices (separately)")
    print(f"Congress: {CONGRESS}")
    print("=" * 70)

    print("Loading clean body data...")
    body_df = pd.read_parquet(BODY_FILE)

    print("Loading clean title data...")
    title_df = pd.read_parquet(TITLE_FILE)

    # --------------------------------------------------
    # ALIGN BY ORIGINAL INDEX (CRITICAL)
    # --------------------------------------------------
    title_df = title_df.loc[body_df.index]
    ...
    print("Fitting bigram vocabulary on BODY text...")
    ...
    # Transform BODY
    # --------------------------------------------------
    print("Transforming BODY text...")
    chunk_size = 50_000
    X_body_chunks = []

    for i in tqdm(range(0, len(texts_body), chunk_size), desc="BODY"):
        X_body_chunks.append(
            vectorizer.transform(texts_body[i:i + chunk_size])
        )
    ...
    # --------------------------------------------------
    print("Transforming TITLE text...")
    X_title_chunks = []

    for i in tqdm(range(0, len(texts_title), chunk_size), desc="TITLE"):
        X_title_chunks.append(
            vectorizer.transform(texts_title[i:i + chunk_size])
        )
